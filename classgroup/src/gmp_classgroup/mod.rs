...
impl GmpClassGroup {
    ...
    // TODO: 看起来更像[Cohen1993, Algorithm 5.4.7], 而不是5.4.9 (NUCOMP).
    fn inner_multiply(&mut self, rhs: &Self, ctx: &mut Ctx) {
        ...
        // k = mu + v*lambda
        // 这一坨似乎对应 [Algorithm 5.4.7] 中的 $$r=y_1y_2n-x_2c_2\bmod v_1$$.
...
impl ClassGroup for GmpClassGroup {
    ...
    fn to_bytes(&self) -> Vec<u8> {
        let mut a_vec: Vec<u8> = (&self.a).into();
        let b_vec: Vec<u8> = (&self.b).into();
        a_vec.extend_from_slice(&b_vec[..]);
        a_vec
    }

    fn from_bytes(bytearray: &[u8], discriminant: Self::BigNum) -> Self {
        let len = bytearray.len();
        let a = Mpz::from(&bytearray[0..166]);
        let b = Mpz::from(&bytearray[166..len]);
        ...
        for _i in 0..10 {
            thread::spawn(|| {
                let s_true = GmpClassGroup {
                    ...
                    c: Mpz::from(36012),
                    discriminant: Mpz::from_str("-3735928559").unwrap(),
                };
                let r = Mpz::from_str("123").unwrap();
                let mut s = GmpClassGroup::new(
                    16.into(),
